...
- `3.py`
  Fusion 360 で直接実行する `グリップ構造` 単体確認用スクリプトです。内部では `内殻` と `外殻基準構造` を参照生成しますが、最終表示には `グリップ構造` のみを残します。
- `fusion_app/`
...
- `fusion_app/grip_structure.py`
  `グリップ構造` の単体確認専用です。まず `内殻` と `外殻基準構造` を生成して `grip.py` の処理を呼び、その後に参照用ボディを消して `グリップ構造` だけを残します。
- `fusion_app/grip.py`
  `グリップ構造` を作る専用です。外部入力値に応じて形状が変わります。
...
- `3.py`
  `グリップ構造` 単体確認用です。参照用に `内殻 + 外殻基準構造` を内部生成したあと、それらを除いた `グリップ構造` だけを確認するために使います。
...
4. `グリップ構造` 単体を見たいときは `3.py` から `fusion_app.grip_structure` を呼ぶ
