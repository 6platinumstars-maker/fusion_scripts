...
- `fusion_app/outer_shell.py`
  `外殻基準構造` を作る専用です。`内殻` を参照して、はめ込む側の基準形状を作ります。
  現在までに作成済みの `外殻` ボディは `外殻基準構造` として扱い、今後はこの基準構造に新しい構造を追加していきます。
- `fusion_app/grip.py`
...
- `外殻`
  `内殻` をはめ込むための別ボディです。現在までに作成済みの本体は `外殻基準構造` として扱い、`内殻` の面や外形を参照して形状を決めます。
  `内殻` と接する面は、`外殻` 側を `0.2 mm` 逃がして隙間を設ける前提です。
- `グリップ`
...
- 現在までの `外殻` 完成形は `外殻基準構造` と名称します。
- `outer_shell.py` では、まず `外殻基準構造` を組み立て、その後はこの基準構造に対して新しい構造を追加していく前提で実装します。
- `外殻` の現在までの仕様は、README の「左手グリップ案件の運用方法」「各ボディの説明」、および各モデル配下の `model_docs/.../README.md` をあわせて参照します。
- 本体は XY 平面の `外殻` スケッチと、内殻断面投影で区切られた外側領域を `-Z` 方向へ `3.0 mm` 押し出して作成します。
- 本体作成後、`内殻` をツールボディとして `Cut` し、内殻の内側範囲を除去します。
- `外殻` と `内殻` が接する `外殻` 側面は `Offset Faces` で `-0.2 mm` オフセットし、接触逃がしを作ります。作られた接触面群には `外殻内殻接面` の名前を付けます。
- 基準点 `(-5.0, -35.0, 0.0) mm` を通る `外殻` ボディ上 XY 面を、`+Z` 方向へ `29.0 mm` `Join` 押し出しします。
- 基準点 `(-5.0, -35.0, -3.0) mm` を通る `外殻` ボディ上 XY 面を、`-Z` 方向へ `0.2 mm` `Join` 押し出しします。
- `z = -3.2 mm` の XY 平面に `外殻底面外側` スケッチを作成し、`J-F-G-H-C-D` 側の閉領域を `-Z` 方向へ `2.8 mm` `Join` 押し出しします。
- `z = -6.0 mm` の XY 平面上で、底面外側追加形状の指定円弧エッジへ `R3.0` フィレットをかけます。
- `内殻蓋部斜面` を法線方向へ `0.2 mm` オフセットした平行平面 `外殻蓋部斜面内部` を作成し、その平面で `外殻` を分割して上側ボディを削除します。
- 原点を通る XY 平面に `外殻外周円` スケッチを作成し、中心 `(-42.0, 5.0, 0.0) mm`、半径 `R33` の円を追加します。
- この円の内側領域を `+Z` 方向へ `40 mm` `Cut` 押し出しし、対象ボディは `外殻` のみに限定します。処理中は `内殻` を一時退避して干渉を避けます。
- 円切り抜き後に `z = 0 mm` の XY 面に残る押し上げ由来の円弧エッジへ `R6.0` フィレットをかけます。
