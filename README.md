...
- `外殻` スケッチは XY 平面に作成します。
- スケッチでは基準点として `A(-5.0, -23.0, 0.0) mm`、`B(-5.0, -35.0, 0.0) mm`、`C(-90.0, -35.0, 0.0) mm`、`D(-90.0, 35.0, 0.0) mm`、`E(-51.8, 35.0, 0.0) mm` を使います。
- `AB` を延長した内殻側接続点を `I(-5.0, -20.0, 0.0) mm` とし、`EF` を延長した内殻側接続点を `J` とします。`J` は `x = -51.8 mm` 上で、内殻上面傾斜線 `(-66.67, 26.918) - (-25.0, 30.0)` と一致する点です。
- 最終的に残す直線は `BI`, `BC`, `CD`, `DE`, `EJ` の 5 本です。
- 内殻の XY 断面ラインは、XY 平面スケッチへ投影したあと通常のスケッチ線へ変換して使います。
- `ABCDE` 側の輪郭と、投影してスケッチ線に変換した内殻アウトラインで区切られた外側領域を `-Z` 方向へ `3.0 mm` 押し出して `外殻` ボディを作ります。
- 押し出し後、`内殻` をツールボディとして `Cut` し、内殻の内側範囲を除去します。
- `外殻` と `内殻` が接する外殻側の面は `Offset Faces` で `-0.2 mm` オフセットし、隙間を作ります。
- このとき作られる接触面群には `外殻内殻接面` という名前を付けて識別します。
