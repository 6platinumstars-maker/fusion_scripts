...
5. 最小プロファイルを負方向へ `Join` 押し出しして段差形状を作る

J 字形状の追加処理は現在入っていません。
...
- `run(context)`
  Fusion 360 から呼ばれる入口です。ベース形状作成から三角形断面の押し出しまでを順に実行します。
...
`core/fusion_helpers.py` には辺を探すための関数があります。
...
`core/fusion_helpers.py` の `find_vertex_by_coordinates()` を使うと、座標ベースで頂点を特定できます。今後必要になれば、頂点にも `add_named_attribute()` で名前を付けられます。
