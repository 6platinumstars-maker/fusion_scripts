...
- `外殻` を蓋部斜面内部平面で分割した後、`Y = 35.0 mm` の `外殻基準構造` 上 XZ 面に `外殻Lボタン開口部` スケッチを追加します。
- このスケッチは H `(-51.8, 35.0, 20.654) mm`、I `(-59.8, 35.0, 20.654) mm`、J `(-59.8, 35.0, -1.346) mm`、K `(-51.8, 35.0, -1.364) mm` の 4 点を結ぶ矩形です。
...
- 矩形内の代表点 `(-55.8, 35.0, 9.6) mm` に最も近い閉領域だけを選択し、`-Y` 方向へ `20 mm` `Cut` 押し出しします。
- この `Y = 35.0 mm` 側の切り取りでも、処理中は `内殻` を一時退避して干渉を避けます。
- 切り取り後の直線エッジのうち、J 点 `(-59.8, 35.0, -1.346) mm` を通る `Y` 軸方向エッジへ `R8.0` フィレットを適用します。
- 続けて、I 点 `(-59.8, 35.0, 20.654) mm` を通る `Y` 軸方向エッジへ `R4.0` フィレットを適用します。
