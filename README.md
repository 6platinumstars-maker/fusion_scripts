...
- `1.py`
  Fusion 360 で直接実行する確認用スクリプトです。現在は `内殻` 単体の最終形確認用入口として使います。
- `2.py`
  Fusion 360 で直接実行する追加の確認用スクリプトです。全体確認や別案比較の入口として使います。
- `3.py`
  Fusion 360 で直接実行する `グリップ構造` 単体確認用スクリプトです。現在は `grip.py` の参照点・参照方向・参照平面を確認する入口として使います。
- `fusion_app/`
  Fusion API を使うアプリ本体の置き場です。案件ごとの完成コードはここに `名前.py` で置きます。
  左手グリップ案件では、`inner_shell.py` / `outer_shell.py` / `grip_structure.py` / `grip.py` / `assembly_left_hand_grip.py` のように役割ごとに分割しています。
- `core/`
...
2. `1.py` `2.py` `3.py` のいずれかから、確認したい `fusion_app/` の入口モジュールを呼ぶ
3. `./sync.sh` を実行する
4. Fusion 360 で `1` `2` `3` のいずれかを実行する
...
- `fusion_app/inner_shell.py`
  `内殻` を作る専用です。現在は左手グリップ案件の内殻生成本体をここで管理します。
- `fusion_app/outer_shell.py`
  `外殻基準構造` を作る専用です。`内殻` を参照して、はめ込む側の基準形状を作ります。
  現在までに作成済みの `外殻` ボディは `外殻基準構造` として扱い、今後はこの基準構造に新しい構造を追加していきます。
- `fusion_app/grip_structure.py`
  `グリップ構造` の単体確認専用です。現在は軽量確認を優先し、参照ボディを生成せずに `grip.py` の処理を呼びます。
- `fusion_app/grip.py`
  `グリップ構造` を作る専用です。現在は `O / G / X / Y / 平面I / ±X / ±Y / ±Z` の参照座標系を表示する初期実装が入っています。
...
- `3.py`
  `グリップ構造` 単体確認用です。現在は参照座標系だけを軽量に確認するために使います。

おすすめの確認手順:

1. `内殻` だけ見たいときは `1.py` から `fusion_app.inner_shell` を呼ぶ
2. `内殻 + 外殻` を見たいときは `2.py` から `fusion_app.assembly_left_hand_grip` を呼ぶ
3. `内殻 + 外殻基準構造 + グリップ構造` を見たいときは `1.py` または `2.py` から `fusion_app.assembly_left_hand_grip` を呼ぶ
4. `グリップ構造` の参照点・参照方向・参照平面を見たいときは `3.py` から `fusion_app.grip_structure` を呼ぶ

`1.py` は `inner_shell.build_inner_shell()` のあとに `inner_shell.add_inner_shell_lid_revolve_cut()` も適用するため、単体確認でも `内殻蓋部回転切り取り` を含む最終形を確認できます。
...
- `グリップ`
  ここでは `グリップ構造` を指します。外部から取得する値に応じて寸法や形状が変わる可変ボディで、`外殻` と重なる部分をカットする参照にも使います。
  現時点ではボディ生成は未実装で、`grip.py` では `グリップ参照点：O / G / X / Y`、`グリップ参照方向：±X / ±Y / ±Z`、`グリップ参照点：平面I` を表示する段階です。

### `fusion_app/`

Fusion API を前提にした実装を置く場所です。左手グリップ案件では `inner_shell.py` / `outer_shell.py` / `grip_structure.py` / `grip.py` / `assembly_left_hand_grip.py` / `helpers.py` / `naming.py` を使います。`Left_hand_grip.py` は旧入口との互換性維持用として残しています。
...
- `model_docs/Left_hand_grip/GRIP_STRUCTURE.md`
...
- `x = -51.8 mm` の YZ オフセット平面に `外殻端修正` スケッチを追加し、A `(-51.8, 35.0, 26.654) mm`、B `(-51.8, 35.0, 20.654) mm`、C `(-51.8, 32.455, 20.654) mm`、D `(-51.8, 32.455, 26.442) mm` を含む範囲を少し広げた矩形を作成します。
- この `外殻端修正` の閉領域を `-X` 方向へ `3 mm` `Cut` 押し出しします。
- `x = -51.8 mm` の YZ オフセット平面に `外殻YZ矩形切り取り` スケッチを追加し、E `(-51.8, 28.0, 28.0) mm`、F `(-51.8, 28.0, -6.0) mm`、G `(-51.8, 32.2, -6.0) mm`、H `(-51.8, 32.2, 28.0) mm` を結ぶ四角形を作成します。
- この `外殻YZ矩形切り取り` の閉領域を `-X` 方向へ `6.0 mm` `Cut` 押し出しします。処理中は `内殻` を一時退避して干渉を避けます。
- `x = -5.0 mm` の YZ オフセット平面に `外殻側面押し出し` スケッチを追加し、E `(-5.0, -23.2, 21.818) mm`、F `(-5.0, -23.3, -6.0) mm`、G `(-5.0, -35.0, -6.0) mm`、H `(-5.0, -35.0, 20.837) mm` を結ぶ四角形を作成します。
- この `外殻側面押し出し` の閉領域を `+X` 方向へ `5 mm` `Join` 押し出しします。
- `x = -9.6 mm` の YZ オフセット平面に `外殻はめ込み微調整` スケッチを追加し、A `(-9.6, -23.2, -3.2) mm`、B `(-9.6, -24.346, -3.2) mm`、C `(-9.6, -24.346, 21.723) mm`、D `(-9.6, -23.2, 21.818) mm` を結ぶ四角形を作成します。
- この `外殻はめ込み微調整` の閉領域を `-X` 方向へ `0.2 mm` `Cut` 押し出しします。
- `外殻` を蓋部斜面内部平面で分割した後、`(-55.8, 35.0, 9.6) mm` を含む `Y = 35.0 mm` の `外殻基準構造` 上 XZ 面全体を `-Y` 方向へ `2.8 mm` `Cut` 押し出しします。処理中は `内殻` を一時退避して干渉を避けます。
- 蓋部斜面隙間部では、主領域の追加とは別に `外殻蓋部斜面隙間部 追加` スケッチから `コントローラ抑え` を作成します。最終仕様は、閉領域を斜面法線負方向へ `3.0 mm` `Join` 押し出しし、終端平行面上の `円弧NM` とそれに接する `PN` / `MF` の 3 エッジへ `R3.0` フィレットを適用する構成です。
- `外殻下部受止` の追加後、蓋部装飾切取りを順に適用します。現在の仕様は、① `x = -41.895 mm` の YZ 平面上 `ABC` 領域を軸 `OP` まわりに `+150° / -120°` 回転 `Cut`、② `外殻蓋部斜面外側` 上の `FG` 基準 90° 傾斜平面で `HIJ` 領域を作り `-X 6.111 mm / +X 5.356 mm` `Cut`、③ ボディ上平面 `A'B'C'` と `A'B'H'` の間を `Cut` ロフト、④ `MNQ` 領域を軸 `KL` まわりに `-120°` 回転 `Cut` したうえで同領域を法線方向へ `20 mm` と反対方向 `1 mm` `Cut` 押し出し、⑤ `XYZ` 領域を `20 mm` と反対方向 `1 mm` `Cut` 押し出しする構成です。
- `外殻下部受止` の段差調整として、`外殻下部受止 スケッチ1` の `A/B` をそれぞれ `(-29.854, -35.0, 23.639) mm` と `(-29.854, -25.9, 24.395) mm` に、`外殻下部受止 スケッチ3` の `H` を `(-28.706, -22.709, 24.652) mm` に更新しています。
- 最終仕上げとして、`x = -90.0 mm` の YZ オフセット平面に `外殻X-90切り取り` スケッチを追加し、Y `-40.0..40.0 mm`、Z `-10.0..35.0 mm` を覆う矩形領域を `+X` 方向へ `10.0 mm` `Cut` 押し出しします。対象ボディは `外殻` のみに限定し、この処理は蓋部装飾切取りと底部外側斜面の完了後に最後に適用します。
- `x = -59.8 mm` の YZ オフセット平面に `外殻Lボタン開口部内部仕様` スケッチを追加し、A `(-59.8, 27.311, 24.969) mm`、B `(-59.8, 32.0, 16.654) mm`、C `(-59.8, 32.0, 11.902) mm`、D `(-59.8, 27.311, -0.69) mm`、E `(-59.8, 31.218, 20.473) mm`、F `(-59.8, 30.747, 4.392) mm` を基準点として使います。
- このスケッチでは `AD` 直線、`BC` 直線、3点円弧 `A-E-B`、3点円弧 `C-F-D` を追加して、`AEBCFD` の半円状閉領域を作成します。
- 外殻断面の交線は位置確認のため同じスケッチへ投影しますが、construction として扱い、回転切り取りのプロファイル分割には使いません。
- `AEBCFD` の半円状閉領域を選択し、軸 `AD` を中心に `360 deg` 回転 `Cut` して外殻Lボタン開口部内部仕様を作成します。
- この回転切り取りでも、対象ボディは `外殻` のみに限定し、処理中は `内殻` を一時退避して干渉を避けます。
- `Y = 32.20 mm` の XZ オフセット平面に `外殻Lボタン開口部 追加` スケッチを追加し、A `(-57.80, 32.20, 24.966) mm`、B `(-57.80, 32.20, 0.0) mm`、C `(-61.254, 32.20, 19.887) mm`、D `(-61.254, 32.20, 12.80) mm` を基準点として使います。
- このスケッチでは `AB` 直線、`DC` 直線、A と C を通って `DC` に接する円弧 `CA`、B と D を通って `DC` に接する円弧 `BD` を追加し、`AB-円弧BD-DC-円弧CA` の閉領域を作成します。
- 外殻断面の交線は位置確認のため同じスケッチへ投影しますが、construction として扱い、押し出し切り取りのプロファイル分割には使いません。
- `AB-円弧BD-DC-円弧CA` の閉領域を選択し、`-Y` 方向へ `15.0 mm` `Cut` 押し出しして追加開口を作成します。
- この押し出し切り取りでも、対象ボディは `外殻` のみに限定し、処理中は `内殻` を一時退避して干渉を避けます。
...
現在の内殻生成本体は `fusion_app/inner_shell.py` です。生成順序は次の通りです。
...
- `build_inner_shell(context=None)`
  `内殻` 本体を生成し、最終ボディを返します。`内殻蓋部回転切り取り` はここでは適用しません。
- `add_inner_shell_lid_revolve_cut(root_comp, target_body=None)`
  `内殻蓋部回転切り取り` を追加で適用します。`1.py` と `assembly_left_hand_grip.py` から使います。
- `run(context)`
  Fusion 360 から呼ばれる入口です。`build_inner_shell()` を実行します。

### `fusion_app/Left_hand_grip.py`

- `run(context)`
  旧入口との互換性維持用です。現在は `fusion_app.inner_shell.run(context)` へ委譲します。
...
`3.py` で参照座標系に加えて、AJ′上の指Aの5本の関節線と参考線AGPを表示します。長さは12 / 25 / 17 / 11 / 14 mmを維持し、角度を近似調整してGからAPに接続します。詳細・生成角度は `model_docs/Left_hand_grip/GRIP_STRUCTURE.md` の「指Aのライン試作」を参照してください。面・ソリッドはまだ生成しません。

### 指Bラインの追加（2026-09-25）

`3.py` で指Aに加えて、BJ′上の指Bの5本の関節線と参考線BGPを表示します。BL0〜BL4は11 / 24 / 21 / 18 / 13 mmを維持し、角度を近似調整してBPに接続します。指Aの長さ表記はAL0〜AL4です。詳細は `model_docs/Left_hand_grip/GRIP_STRUCTURE.md` を参照してください。

### 指C・Dの基準点・基準平面の最終確定（2026-09-25）

`3.py` でCP・CP′・DP・DP′と、CJ・CJ′・DJ・DJ′を表示します。J平面はG・P・P′を通り、関節用J′平面はG・Pを通ってJに直交します。指C・Dの基準点・基準平面はFusionで確認済みで、この構成を最終確定とします。関節線はCJ′・DJ′上に作成する方針で、長さ・角度の指定と線の生成は今後の作業です。確定座標は `model_docs/Left_hand_grip/GRIP_STRUCTURE.md` に記載しています。
