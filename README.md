...
- `fusion_app/`
  ...
  左手グリップ案件では、`inner_shell.py` / `outer_shell.py` / `grip.py` / `assembly_left_hand_grip.py` のように役割ごとに分割しています。
- `core/`
...
Fusion API を前提にした実装を置く場所です。左手グリップ案件では `inner_shell.py` / `outer_shell.py` / `grip.py` / `assembly_left_hand_grip.py` / `helpers.py` / `naming.py` を使います。`Left_hand_grip.py` は旧入口との互換性維持用として残しています。
