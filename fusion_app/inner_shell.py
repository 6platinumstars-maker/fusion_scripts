...
INNER_SHELL_OUTER_PERIMETER_BODY_NAME = '内殻外周'
...
INNER_SHELL_LID_REVOLVE_CUT_PLANE_X_MM = -41.86
INNER_SHELL_LID_REVOLVE_CUT_POINT_A_MM = (-41.86, -28.353, 22.2)
INNER_SHELL_LID_REVOLVE_CUT_POINT_B_MM = (-41.86, -28.358, 20.0)
INNER_SHELL_LID_REVOLVE_CUT_POINT_C_MM = (-41.86, -25.617, 21.216)
INNER_SHELL_LID_REVOLVE_CUT_POINT_D_MM = (-41.86, -21.614, 20.355)
INNER_SHELL_LID_REVOLVE_CUT_POINT_E_MM = (-41.86, -21.619, 22.2)
INNER_SHELL_LID_REVOLVE_CUT_AXIS_X_MM = (-41.86, 5.062, 23.966)
INNER_SHELL_LID_REVOLVE_CUT_AXIS_Y_MM = (-41.86, 5.695, 15.591)
...
INNER_SHELL_LID_POINT_REFS = {
    name: create_point_mm(*coords_mm)
    for name, coords_mm in INNER_SHELL_LID_POINT_REFS_MM.items()
}


def create_offset_plane_from_yz(root_comp, x_mm, name):
    planes = root_comp.constructionPlanes
    plane_input = planes.createInput()
    plane_input.setByOffset(
        root_comp.yZConstructionPlane,
        adsk.core.ValueInput.createByReal(mm_to_cm(x_mm)),
    )
...
def to_sketch_space(sketch, *args):
    if len(args) == 1:
        model_point = create_point_mm(*args[0])
    elif len(args) == 3:
        model_point = adsk.core.Point3D.create(args[0], args[1], args[2])
    else:
        raise ValueError('to_sketch_space expects either one point tuple or x, y, z values.')
...
def get_largest_profile(sketch):
    ...
    if largest_profile is None:
        raise RuntimeError('内殻蓋部回転切り取りのプロファイルを取得できませんでした。')
...
def add_named_attribute(entity, name):
    helpers.add_named_attribute(entity, name)
...
def create_base_sketch(root_comp):
    ...
    point_1 = adsk.core.Point3D.create(x_right, y_front, 0.0)
    point_2 = adsk.core.Point3D.create(x_left, y_front, 0.0)
    point_3 = adsk.core.Point3D.create(x_left, slope_end_y, 0.0)
    point_4 = adsk.core.Point3D.create(slope_start_x, slope_start_y, 0.0)
    point_5 = adsk.core.Point3D.create(x_right, y_back, 0.0)
...
def create_split_triangle_on_face(root_comp, face, fusion_helpers):
    split_sketch = fusion_helpers.create_sketch_on_face(root_comp, face, '分割断面')
    add_named_attribute(face, '分割断面')
    fusion_helpers.project_face_edges(split_sketch, face)
...
def get_profiles_nearest_points(sketch, target_points):
    ...
    for target_x, target_y in target_points:
        ...
        if nearest_profile is None:
            raise RuntimeError('指定座標に対応するプロファイルを取得できませんでした。')

        if nearest_profile.entityToken in selected_tokens:
            raise RuntimeError('プロファイルが期待どおりに分離されていませんでした。')

        selected_tokens.add(nearest_profile.entityToken)
...
def create_outer_bottom_semicircle_sketch(root_comp, face, fusion_helpers):
    sketch = fusion_helpers.create_sketch_on_face(root_comp, face, '止金半円部')
    fusion_helpers.project_face_edges(sketch, face)
...
def create_retainer_outer_fan_sketch(root_comp, face, fusion_helpers):
    sketch = fusion_helpers.create_sketch_on_face(root_comp, face, '留金部外部 ')
...
def create_joystick_receiver_sketch(root_comp):
    ...
    sketch.sketchCurves.sketchCircles.addByCenterRadius(center_point, 1.5)
    return sketch
...
def create_inner_shell_outer_perimeter_sketch(root_comp):
    ...
    center_point = adsk.core.Point3D.create(-4.2, 0.5, 0.0)
    upper_circle_point = adsk.core.Point3D.create(-2.8579, -2.2944, 0.0)
    target_point = adsk.core.Point3D.create(-2.9842, -2.5679, 0.0)
    line_start_point = adsk.core.Point3D.create(-6.667, 2.6918, 0.0)
    line_end_point = adsk.core.Point3D.create(-2.5, 3.0, 0.0)

    circles.addByCenterRadius(center_point, 3.1)
    circles.addByCenterRadius(center_point, 3.3)
    lines.addByTwoPoints(upper_circle_point, target_point)
    lines.addByTwoPoints(line_start_point, line_end_point)
    return sketch


def create_outer_shell_option2_cut_sketch(root_comp, face, fusion_helpers):
    sketch = fusion_helpers.create_sketch_on_face(root_comp, face, '内殻外部_仕様案2')
    fusion_helpers.project_face_edges(sketch, face)
...
def find_face_by_named_attribute(body, name):
    for face in body.faces:
        attribute = face.attributes.itemByName(naming.ATTRIBUTE_GROUP, naming.ATTRIBUTE_NAME_KEY)
...
def add_inner_shell_lid_revolve_cut(root_comp, target_body=None):
    if root_comp is None:
        raise RuntimeError('root_comp is required.')

    if target_body is None:
        target_body = helpers.find_body_by_name_or_attribute(
            root_comp,
            INNER_SHELL_OUTER_PERIMETER_BODY_NAME,
        )
    if target_body is None:
        target_body = helpers.find_body_by_name_or_attribute(
            root_comp,
    ...
    if target_body is None:
        raise RuntimeError('内殻蓋部回転切り取りの対象ボディを取得できませんでした。')

    plane = create_offset_plane_from_yz(
        root_comp,
        INNER_SHELL_LID_REVOLVE_CUT_PLANE_X_MM,
        '内殻蓋部回転切り取り平面',
    )
    sketch = root_comp.sketches.add(plane)
    sketch.name = '内殻蓋部回転切り取り'

    lines = sketch.sketchCurves.sketchLines
    point_a = sketch.sketchPoints.add(to_sketch_space(sketch, INNER_SHELL_LID_REVOLVE_CUT_POINT_A_MM))
    point_b = sketch.sketchPoints.add(to_sketch_space(sketch, INNER_SHELL_LID_REVOLVE_CUT_POINT_B_MM))
    point_c = sketch.sketchPoints.add(to_sketch_space(sketch, INNER_SHELL_LID_REVOLVE_CUT_POINT_C_MM))
    point_d = sketch.sketchPoints.add(to_sketch_space(sketch, INNER_SHELL_LID_REVOLVE_CUT_POINT_D_MM))
    point_e = sketch.sketchPoints.add(to_sketch_space(sketch, INNER_SHELL_LID_REVOLVE_CUT_POINT_E_MM))
    point_x = sketch.sketchPoints.add(to_sketch_space(sketch, INNER_SHELL_LID_REVOLVE_CUT_AXIS_X_MM))
    point_y = sketch.sketchPoints.add(to_sketch_space(sketch, INNER_SHELL_LID_REVOLVE_CUT_AXIS_Y_MM))

    lines.addByTwoPoints(point_a, point_b)
    lines.addByTwoPoints(point_b, point_c)
    lines.addByTwoPoints(point_c, point_d)
    lines.addByTwoPoints(point_d, point_e)
    lines.addByTwoPoints(point_e, point_a)
    axis_line = lines.addByTwoPoints(point_x, point_y)
...
def _build_inner_shell_with_legacy_api(root_comp, fusion_helpers, app_helpers):
    ...
    base_profile = fusion_helpers.get_largest_profile(base_sketch)
    ...
    body = fusion_helpers.get_body_from_feature(base_feature)
    app_helpers.set_body_identity(body, naming.BODY_INNER_SHELL)

    split_face = fusion_helpers.find_face_by_axis_value(body, 'x', 0.0)
    split_sketch = create_split_triangle_on_face(root_comp, split_face, fusion_helpers)
    ...
    inner_shell_lid_profile = fusion_helpers.get_largest_profile(inner_shell_lid_slope_cut_sketch)
    ...
    joystick_receiver_outer_body = fusion_helpers.get_body_from_feature(joystick_receiver_outer_feature)
    ...
    inner_shell_outer_body = fusion_helpers.get_body_from_feature(inner_shell_outer_feature)
    add_named_attribute(inner_shell_outer_body, INNER_SHELL_OUTER_PERIMETER_BODY_NAME)
    ...
    inner_shell_lid_face_result = create_inner_shell_lid_face_sketch(
        root_comp,
    ...
    top_face = fusion_helpers.find_face_by_axis_value(body, 'z', 0.0)
    outer_shell_option2_sketch = create_outer_shell_option2_cut_sketch(
        root_comp,
        top_face,
        fusion_helpers
    )
    ...
    upper_stop_profile = fusion_helpers.get_largest_profile(upper_stop_sketch)
    ...
    upper_stop_body = fusion_helpers.get_body_from_feature(upper_stop_feature)
    ...
    lower_stop_profile = fusion_helpers.get_largest_profile(lower_stop_sketch)
    extrude_profile(
        root_comp,
        lower_stop_profile,
        1.0,
    ...
    lower_stop_face = fusion_helpers.find_face_by_axis_value(body, 'y', -2.0)
    ...
    body = root_comp.bRepBodies.item(0)
    outer_bottom_semicircle_center = create_point_mm(0.0, 5.0, -3.0)
    ...
    joystick_receiver_profile = fusion_helpers.get_largest_profile(joystick_receiver_sketch)
    extrude_profile(
        root_comp,
    ...
    outer_shell_cut_profile = fusion_helpers.get_largest_profile(outer_shell_cut_sketch)
    extrude_profile(
        root_comp,
...
def build_inner_shell(context=None):
    ...
    fusion_helpers = helpers.load_fusion_helpers()
    return _build_inner_shell_with_legacy_api(root_comp, fusion_helpers, helpers)
